...
function MicroServicesModule(opts) {
    ...
    this.options = opts || (opts = minimist(process.argv.slice(2), {
        default: {
            /**
    ...
    this.useTransport = function(value, options) {
        ...
        process.on('SIGINT', disposeThenExit);
        transport = value;
        ...
        function disposeTransport() {
            if (transport === value)
                transport = undefined;
            value.stop();
            process.removeListener('SIGINT', disposeThenExit);
        }
    };
